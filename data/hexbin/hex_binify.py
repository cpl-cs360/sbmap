...
def compute(i, total):
    state = {
        ...
        'ids': [[] for _ in range(len(hexes.get_array()))],           # IDS_ARR init to empty sets
        'orbits': []                                                  # ORBITS_ARR
    }
    ...
    for index, row in tqdm(df.iterrows(), total=df.shape[0]):
        ...
        for j in range(len(set_of_bins)):
            curr = set_of_bins[j]
            state['count'][j] += curr                         # counts[j] += curr
...
if __name__ == '__main__':
    ...
    with concurrent.futures.ProcessPoolExecutor() as executor:
        ...
        for f in concurrent.futures.as_completed(results):
            ...
            for i in range(len(resultState['count'])):
                aggregate_count_arr[i] += resultState['count'][i]         # update current bin's count
